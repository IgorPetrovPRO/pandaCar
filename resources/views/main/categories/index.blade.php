...
@section('subhead')
    <title>{{ __('categories.title') }}</title>
@endsection

@section('subcontent')
    <h2 class="intro-y text-lg font-medium mt-10">{{ __('categories.h1') }}</h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{route('categories.create')}}" class="btn btn-primary shadow-md mr-2">{{ __('categories.add') }}</a>
                    ...
                    <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                        <div class="w-56 relative text-slate-500">
                            <input type="text" name="text" class="form-control w-56 box pr-10"
                                   placeholder="{{ __('categories.search') }}" value="{{request()->text}}"
        ...
        @if($categories->total() > 0)
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                ...
                <table class="table table-report -mt-2">
                    ...
                    <tbody>
                    @each('main.categories.partials.row' , $categories ,'category')
                    </tbody>
                </table>
            </div>
            <!-- END: Data List -->
            ...
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
                {{$categories->links('layout.components.pages.links')}}
            </div>
            <!-- END: Pagination -->
        @else
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
